...
public class Monitor { 
    ...
    //Aplicaci贸n de Singleton.
    private static final Monitor instance = new Monitor();
	 private Monitor(){
		//Sem谩foro binario a la entrada del monitor.
	...
	public void configRdp(String path){
        ...
        for(int i=0;i<this.getNumeroTransiciones();i++){ 
            colas[i]=new Cola(); //Inicializaci贸n de colas.
        }
		mutex.release();
	}
...
public List<Integer> andVector(List<Integer> lista1, List<Integer> lista2) throws IndexOutOfBoundsException{
	if (lista1.size()==lista2.size()) {
		...
		throw new IndexOutOfBoundsException("Listas de diferentes tama帽os");
	}
		...
		while(k){
			...
			if(k){ //K=true verifica el estado de la red.
				...
				if(isNotAllZeros(m)){
					...
					try{
						colas[transicionADisparar].resume(); //Sale de una cola de condici贸n.
					}
				...
				catch(Exception e){ //Puede haber mas de un tipo de excepcion. (Por interrupci髇 o por exceder los l韒ites).
					e.printStackTrace();
				}
